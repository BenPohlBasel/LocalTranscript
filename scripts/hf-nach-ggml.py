#!/usr/bin/env python3
# Hugging-Face-Whisper-Feintuning → ggml für whisper.cpp (Eigene Modelle,
# Ordner <Bibliothek>/Modelle/). Kopie von whisper.cpp/models/
# convert-h5-to-ggml.py (MIT, ggml-org, Stand 2026-09-11) mit EINER
# Änderung, markiert «PATCH»: die Tokentabelle wird nach ID indiziert
# geschrieben. Das Original schreibt sie fortlaufend — richtig für
...
#   mv aus/ggml-model.bin ~/Documents/LocalTranscript/Modelle/ggml-crisperwhisper.bin
...
# PATCH (LocalTranscript 2026-09-11): Tokens nach ID indizieren, nicht als
# fortlaufende Liste. CrisperWhisper hat einen neu trainierten Tokenizer
# (45066 Tokens, ids 0..50257 mit 5192 Lücken — Tokens ohne führendes
# Leerzeichen); die Original-Schleife schreibt sortiert nach id und
# whisper.cpp nimmt die POSITION als id → 44810 Tokens falsch → Kauderwelsch.
nach_id = {i: k for k, i in tokens.items() if i < encoder_added.get("<|endoftext|>", 50257)}
n_basis = max(nach_id) + 1
fout.write(struct.pack("i", n_basis))
for i in range(n_basis):
    text = bytearray([byte_decoder[c] for c in nach_id.get(i, "")])
    fout.write(struct.pack("i", len(text)))
    fout.write(text)
print("Tokens nach ID geschrieben:", n_basis, "davon Lücken:", n_basis - len(nach_id))
